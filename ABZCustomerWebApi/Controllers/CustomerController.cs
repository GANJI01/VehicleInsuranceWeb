        ...
        public async Task<ActionResult> Insert(Customer customer)
        {
            try
            {
               ...
               await client2.PostAsJsonAsync("http://localhost:5083/api/Vehicle/Customer/", new { customerId = customer.CustomerID });
                ...
                //await client2.PostAsJsonAsync("http://abzvehiclewebapi-hana.azurewebsites.net/api/vehicle/Customer", new { customerId = customer.CustomerID });
                //await client.PostAsJsonAsync("http://abzproposalwebapi-hana.azurewebsites.net/api/Proposal/Customer", new { customerId = customer.CustomerID });
